...
class AdminApproveMemberApplicationAction
{
    ...
    public function execute(
        ...
        if (! in_array((string) $memberApplication->affiliation_status, ['validated', 'rejected'], true)) {
            throw ValidationException::withMessages([
                'affiliation_status' => ['Association affiliation decision is required before final approval.'],
            ]);
        }
        ...
        return DB::transaction(function () use ($memberApplication, $reviewer, $comment, $ipAddress, $userAgent) {
            ...
            SendMemberApprovalConfirmationJob::dispatch($freshApplication, $freshMember->member_code)->afterCommit();
