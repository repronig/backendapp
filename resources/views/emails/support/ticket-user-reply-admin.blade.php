...
@slot('subject') New reply on ticket {{ \App\Models\SupportTicket::formattedReference((int) ($ticket?->id ?? 0)) }} @endslot
...
<p style="margin:0 0 18px; font-size:16px; line-height:1.7; color:#4c4c4c;">
    ...
    @else
        A user
    @endif
    added a reply on ticket <strong>{{ \App\Models\SupportTicket::formattedReference((int) ($ticket?->id ?? 0)) }}</strong>
    @if($ticket?->subject)
