...
{
    public const DEMO_MEMBER_EMAIL = 'member.demo@repronig.com';

    public const DEMO_MEMBER_PASSWORD = 'Password123!';

    public const DEMO_INSTITUTION_USER_EMAIL = 'institution.user@repronig.com';

    public const DEMO_INSTITUTION_PASSWORD = 'Password123!';

    public function run(): void
    {
        DB::transaction(function (): void {
            $this->seedCanonicalPortalUsers();
        });
    ...
    protected function seedCanonicalPortalUsers(): void
    {
        $association = Association::query()->where('code', 'NPA')->firstOrFail();
        $associationReviewer = User::query()->where('email', 'info@npa.org')->firstOrFail();
        $adminReviewer = User::query()->where('email', 'admin@repronig.org')->firstOrFail();
        ...
        if (! $memberUser->member) {
            $application = app(CreateMemberApplicationAction::class)->execute(
                ...
                [
                    ...
                    'next_of_kin_phone' => '+2348000000015',
            ...
            foreach (['proof_of_id', 'proof_of_address'] as $index => $documentType) {
                MemberApplicationDocument::query()->firstOrCreate([
                    ...
                    'file_path' => 'documents/demo-'.$documentType.'.pdf',
            ...
            app(ApproveMemberApplicationAction::class)->execute(
                $submitted,
                $associationReviewer,
                'Seed demo member affiliation validated.',
                '127.0.0.1',
                'DemoDataSeeder'
            );

            app(AdminApproveMemberApplicationAction::class)->execute(
                $submitted->fresh(),
                $adminReviewer,
        ...
        $institutionUser = User::query()->updateOrCreate(
            ['email' => self::DEMO_INSTITUTION_USER_EMAIL],
            [
                'first_name' => 'John',
                'last_name' => 'Registrar',
                'phone' => '+2348000000016',
                'password' => Hash::make(self::DEMO_INSTITUTION_PASSWORD),
        ...
        InstitutionProfile::query()->updateOrCreate(
            ['institution_id' => $institution->id],
            InstitutionProfile::factory()->make(['institution_id' => $institution->id])->toArray()
        );
    }
